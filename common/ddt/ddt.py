...
DATA_ATTR = '%values'  # store the element the test must run with
...
def file_data(value, yaml_loader=None):
    ...
    should contain JSON encoded element, that can either be a list or a
    dict.
    ...
    test case, and values will be fed as test element.
...
def mk_test_name(name, value, index=0, name_fmt=TestNameFormat.DEFAULT):
    ...
    test_name = "{0}_{1}".format(name, index, value)
    return re.sub(r'\W|^(?=\d)', '_', test_name)


def feed_data(func, new_name, test_data_docstring, *args, **kwargs):
    """
    This internal method decorator feeds the test element item to the test.
...
def process_file_data(cls, name, func, file_attr):
    ...
    with codecs.open(data_file_path, 'r', 'utf-8') as f:
        # Load the element from YAML or JSON
        if _is_yaml_file:
...
def _add_tests_from_data(cls, name, func, data):
    """
    Add tests from element loaded from the element file into the class
    """
...
def ddt(arg=None, **kwargs):
    ...
    decorate test methods with ``@element``.

    For each method decorated with ``@element``, this will effectively create as
    many methods as element items are passed as parameters to ``@element``.

    The names of the test methods follow the pattern
    ``original_test_name_{ordinal}_{element}``. ``ordinal`` is the position of the
    element argument, starting with 1.

    For element we use a string representation of the element value converted into a
    valid python identifier.  If ``element.__name__`` exists, we use that instead.

    For each method decorated with ``@file_data('test_data.json')``, the
    decorator will try to load the test_data.json file located relative
    to the python file containing the method that is decorated. It will,
    for each ``test_name`` key create as many methods in the list of values
    from the ``element`` key.
